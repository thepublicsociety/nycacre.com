class BackgroundsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @background.save
        @background.update_attribute("height", Paperclip::Geometry.from_file(@background.background_image.path).height.to_i)
        @background.update_attribute("width", Paperclip::Geometry.from_file(@background.background_image.path).width.to_i)
        format.html { redirect_to backgrounds_url, notice: 'Background was successfully created.' }
  ...
  def update
    ...
    respond_to do |format|
      if @background.update_attributes(params[:background])
        format.html { redirect_to backgrounds_url, notice: 'Background was successfully updated.' }
